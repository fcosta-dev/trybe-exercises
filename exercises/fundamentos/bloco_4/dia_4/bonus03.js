...
for (let index = 0; index < basket.length; index += 1) {
  ...
  if (!listaOrganizada[fruit]) listaOrganizada[fruit] = 0;
  listaOrganizada[fruit] += 1;
}

const summaries = [];
for (fruit in listaOrganizada) {
  let message = `${listaOrganizada[fruit]} ${fruit}`;
  if (listaOrganizada[fruit] > 1) message += 's';
